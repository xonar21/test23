...
namespace GR.Identity.Services
{
    ...
    public class LocationService : ILocationService
    {
        ...
        /// </summary>
        public virtual async Task<ResultModel<PagedResult<CountryViewModelIdentity>>> GetAllPaginatedCountriesAsync(PageRequest request)
        {
            var cities = await _context.Countries
                .AsQueryable()
                .GetPagedAsync(request);

            var map = cities.Map(_mapper.Map<IEnumerable<CountryViewModelIdentity>>(cities.Result));

            return new SuccessResultModel<PagedResult<CountryViewModelIdentity>>(map);
        }
    }
}