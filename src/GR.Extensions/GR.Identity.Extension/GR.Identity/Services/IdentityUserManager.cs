...
namespace GR.Identity.Services
{
    ...
    public class IdentityUserManager : IUserManager<GearUser>
    {
        ...
        public IdentityUserManager(UserManager<GearUser> userManager, IHttpContextAccessor httpContextAccessor,
            RoleManager<GearRole> roleManager, IIdentityContext identityContext)
        {
        ...
        public virtual Guid? CurrentUserTenantId
        {
            get
            {
                Guid? val = _httpContextAccessor?.HttpContext?.User?.Claims?.FirstOrDefault(x => x.Type == "tenant")
                    ?.Value
                ...
                val = userClaims?.FirstOrDefault(x => x.Type == "tenant" && !string.IsNullOrEmpty(x.Value))?.Value
                    ?.ToGuid();
        ...
        public virtual async Task<ResultModel> AddToRolesAsync(GearUser user, ICollection<string> roles)
        {
            ...
            var defaultRoles = new Collection<string> {GlobalResources.Roles.USER};
            ...
            var exist = new GearUser();

            if (!user.Email.IsNull())
                exist = await UserManager.FindByEmailAsync(user.Email);
            else
                exist = await UserManager.FindByNameAsync(user.UserName);
        ...
        public virtual async Task<IEnumerable<GearRole>> GetUserRolesAsync(GearUser user)
        {
            ...
            {
                var roleToUpdate = RoleManager.Roles.FirstOrDefault(x => x.Name == role);
                if (roleToUpdate.NormalizedName != role.ToUpper())
                {
        ...
        /// <returns></returns>
        public async Task<ResultModel<IEnumerable<SampleGetUserViewModel>>> GetUsersInRoleForCurrentCompanyAsync(
            [Required] string roleName)
        {
            if (roleName.IsNullOrEmpty())
                return new InvalidParametersResultModel<IEnumerable<SampleGetUserViewModel>>();
            ...
            var filterUsers = allUsers.Where(x => x.TenantId.Equals(currentUserRequest.Result.TenantId))
                .Select(x => new SampleGetUserViewModel(x)).ToList();
        ...
        public virtual ResultModel<Guid> FindUserIdInClaims()
        {
            ...
            {
                result.Errors.Add(new ErrorModel {Message = "Unauthorized user"});
                return result;
            }
        ...
        /// <returns></returns>
        public async Task<ResultModel<IEnumerable<GearUser>>> GetUsersInRolesAsync(IEnumerable<GearRole> roles,
            Guid? tenantId = null)
        {
        ...
        public bool IsAuthenticated()
            => _httpContextAccessor?.HttpContext?.User?.IsAuthenticated() ?? false;
    }
