...
namespace GR.Identity.Services
{
    public class UserAddressService : IUserAddressService
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<ResultModel<Country>> GetCountryByIdAsync(string id)
        {
            if(id == null)
                return  new InvalidParametersResultModel<Country>();

            var country = await _context.Countries
                .FirstOrDefaultAsync(x => x.Id.ToString() == id);

            if(country == null)
                return new NotFoundResultModel<Country>();
        ...
        /// <returns></returns>
        public async Task<ResultModel<StateOrProvince>> GetCityByIdAsync(int? cityId)
        {
            if(!cityId.HasValue)
                return  new InvalidParametersResultModel<StateOrProvince>();

            var city = await _context.StateOrProvinces
                .FirstOrDefaultAsync(x => x.Id == cityId.Value);

            if(city == null)
                return  new NotFoundResultModel<StateOrProvince>();
        ...
        /// <returns></returns>
        public async Task<ResultModel<District>> GetDistrictByIdAsync(Guid? districtId)
        {
            if(districtId == null)
                return new InvalidParametersResultModel<District>();

            var district = await _context.Districts
                .FirstOrDefaultAsync(x => x.Id == districtId);

            if(district == null)
                return  new NotFoundResultModel<District>();

            return  new SuccessResultModel<District>(district);
        }
    }
}