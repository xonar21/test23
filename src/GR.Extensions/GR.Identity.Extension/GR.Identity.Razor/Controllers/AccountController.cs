...
namespace GR.Identity.Razor.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        /// Inject UserManager
        /// </summary>
        protected readonly UserManager<GearUser> _identityUserManager;


        /// <summary>
        /// Inject role manager
        /// </summary>
        protected readonly RoleManager<GearRole> _roleManager;
        ...
        public AccountController(
            ...
            UserManager<GearUser> identityUserManager,
            RoleManager<GearRole> roleManager,
            ILdapAuthorizeService ldapAuthorizeService)
        {
        ...
        public async Task<IActionResult> Account()
        {
            ...
            if (!curentUser.IsSuccess) return NotFound();
            return View(curentUser.Result);
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Login(Abstractions.ViewModels.AccountViewModels.LoginViewModel model, string returnUrl = null)
        {
            // http://ad.indrivo.com/
            ...
            var user = await _userManager.UserManager.Users.FirstOrDefaultAsync(x => x.UserName == model.EmailAddress);
            if(user == null)
            {
                user = await _userManager.UserManager.Users.FirstOrDefaultAsync(x => x.Email == model.EmailAddress);
            }
        ...
        {
            var userClaims = await _applicationDbContext.UserClaims.Where(x => x.UserId == user.Id.ToString()).ToListAsync();
            if (userClaims.Any())
            {
        ...
        {
            ViewData[ReturnUrl] = returnUrl;
            var allUsers = _ldapService.GetAllUsers();
            if (!ModelState.IsValid) return View(model);
            var user = new LdapUser { Name = model.Email, EmailAddress = model.Email, FirstName = model.FirstName, LastName = model.LastName };
...
*/
            ...
            _ldapService.AddUser(user, model.Password);
        ...
        private async Task<ResultModel<Abstractions.ViewModels.AccountViewModels.LoginViewModel>> ExtractAdUserAsync(string username, string password, string returnUrl = null)
        {
            var userAd = _ldapService.GetUserByUserName(username);
            if (userAd == null)
                return new ResultModel<Abstractions.ViewModels.AccountViewModels.LoginViewModel>
                {
                    IsSuccess = false,
                    Errors = new List<IErrorModel> {new ErrorModel
                            { Message = "User not fount in AD" } }
                };

            var test = _ldapService.Authenticate(userAd.DistinguishedName, password);
            ...
            if (!test)
            {
                ...
                {
                    IsSuccess = false,
                    Errors = new List<IErrorModel> { new ErrorModel { Message = "Invalid credentials" } }
                };
            }
            ...
            var user = new GearUser
            {
                ...
                UserFirstName = userAd.FirstName,
                UserLastName = userAd.LastName,
                Birthday = userAd.Birthday,
                AboutMe = userAd.AboutMe,
                PhoneNumber = userAd.PhoneNumber,
            };
            ...
            if (!result.Succeeded)
            {
                return new ResultModel<Abstractions.ViewModels.AccountViewModels.LoginViewModel>
                {
                    IsSuccess = false,
                    Errors = new List<IErrorModel> {new ErrorModel
                            { Message = "Error save AD user" } }
                };
            }
            var roleRequest = await _roleManager.Roles.FirstOrDefaultAsync(x => x.Name == "User");
            if (roleRequest != null)
                await _identityUserManager.AddToRoleAsync(user, roleRequest.Name);


            var loginResult =
                await _signInManager.PasswordSignInAsync(user.UserName, password, false,
                    false);
            ...
            if (result.Succeeded)
            {
                ...
                return new SuccessResultModel<Abstractions.ViewModels.AccountViewModels.LoginViewModel>();
            }
