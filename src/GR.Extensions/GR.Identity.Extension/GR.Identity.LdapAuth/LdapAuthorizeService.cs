...
namespace GR.Identity.LdapAuth
{
    ...
    public class LdapAuthorizeService : ILdapAuthorizeService
    {
        ...
        public LdapAuthorizeService(SignInManager<GearUser> signInManager, IUserManager<GearUser> userManager, ILogger<LdapAuthorizeService> logger, IHttpContextAccessor contextAccessor, BaseLdapUserManager<LdapUser> ldapUserManager, ILdapService<LdapUser> ldapService, LdapConfiguration ldapConfiguration)
        {
        ...
        public virtual async Task<ResultModel> LoginAsync(GearUser user)
        {
            ...
            if (user == null)
            {
                response.Errors.Add(new ErrorModel(string.Empty, "User not found"));
                return response;
            }
            if (user.IsDeleted)
            {
            ...
            if (userAd == null)
            {
                response.Errors.Add(new ErrorModel(string.Empty, "User not found in AD"));
                return response;
            }
        ...
        public virtual async Task<ResultModel> LoginAsync(LoginViewModel model)
        {
            ...
            var userAd = _ldapService.GetUserByUPN(model.EmailAddress);
            ...
            var user = await _userManager.UserManager.FindByEmailAsync(model.EmailAddress);

            if (user == null && _ldapConfiguration.AutoImportOnLogin)
            {
                var importResult = await _ldapUserManager.ImportAdUserAsync(model.EmailAddress, model.Password);
                if (importResult.IsSuccess)
                {
                    user = await _userManager.UserManager.FindByEmailAsync(model.EmailAddress);
                }
            }
            ...
            if (user != null)
            {
                ...
                if (user.IsPasswordExpired() && !await _userManager.UserManager.IsInRoleAsync(user, GlobalResources.Roles.ADMINISTRATOR))
                {
                    response.Errors.Add(new ErrorModel(string.Empty, "Password has been expired, you need to change the password"));
                }
        ...
        {
            var response = new ResultModel();
            var user = await _userManager.UserManager.Users.FirstOrDefaultAsync(x => x.Email == model.EmailAddress);
            if (user == null)
            {
                response.Errors.Add(new ErrorModel(string.Empty, "User not found"));
                return response;
            ...
            var userAd = _ldapService.GetUsersByEmailAddress(model.EmailAddress).First();
            if (userAd == null)
            {
                response.Errors.Add(new ErrorModel(string.Empty, "User not found in AD"));
                return response;
