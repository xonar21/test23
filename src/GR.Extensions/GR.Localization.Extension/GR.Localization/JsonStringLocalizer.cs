...
namespace GR.Localization
{
    public class JsonStringLocalizer : IStringLocalizer
    {
            ...
            ICacheService cache)
        {
            ...
            _language = httpAccessor.HttpContext.Request.GetTypedHeaders()
           .AcceptLanguage
           ?.OrderBy(x => x.Quality)
           ...
           .Split("-").FirstOrDefault();
            ...
            httpAccessor.HttpContext.Response.Cookies.Append("language", new CultureInfo(_language).DisplayName);
        }
        #endregion
        ...
        private string GetFilePath()
        {
            ...
            var resultPaths = Path.Combine(paths);
            return resultPaths;
        }
        /// <summary>
        /// Checks if there is a file with translation according to language ISO provided
        /// </summary>
        /// <returns>Bool true if file exists, else false </returns>
        private bool TranslationExists()
        {
            var path = string.Format("{0}/{1}{2}", _path, _language, ".json");
