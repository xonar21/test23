...
namespace GR.MultiTenant.Services
{
    public class OrganizationService : IOrganizationService<Tenant>
    {
        ...
        public virtual IEnumerable<GearUser> GetUsersByOrganization(Guid organizationId, Guid roleId)
        {
            ...
            return _context.Users.Where(x => x.TenantId == organizationId && usersId.Contains(x.Id.ToString()));
        }
        ...
        public virtual async Task SendInviteToEmailAsync(GearUser user)
        {
            ...
            var callbackUrl = _httpContextAccessor.HttpContext.Request.Scheme + "://" +
                              _httpContextAccessor.HttpContext.Request.Host.Value + "/Company/ConfirmInvitedUserByEmail" +
                               "?userId=" + user.Id + "&confirmToken=" + code;
