...
namespace GR.Crm.Organizations
{
    public class ContactService : ICrmContactService
    {
        ...
        ///<summary>
        ///Inject PhoneList service
        /// </summary>
        private readonly ICrmPhoneListService _phoneListService;
        ...
        public ContactService(ICrmOrganizationContext organizationContext,
            IMapper mapper,
            ICrmPhoneListService phoneListService,
        ...
        public virtual async Task<ResultModel<PagedResult<PaginateContactViewModel>>> GetAllContactsPaginatedAsync(PageRequest request)
        {
            ...
            var clearOldContexResult = await _organizationContext.PushAsync();

            foreach (var contact in query.Result)
            {
        ...
        public virtual async Task<ResultModel<Guid>> UpdateContactAsync(ContactViewModel model)
        {
            ...
            if(await _organizationContext.JobPositions.FirstOrDefaultAsync(x=> x.Id == model.JobPositionId) != null) contact.JobPositionId = model.JobPositionId;
            contact.OrganizationId = model.OrganizationId;
            contact.Description = model.Description;
            contact.NotAvailable = model.NotAvailable;
        ...
        public virtual async Task<ResultModel> DeleteContactAsync(Guid? contactId)
        {
            ...
            var emails = await _emailContext.Emails.Where(x => x.ContactId == contactId).ToListAsync();
            if (emails.Count() > 0)
            {
            ...
            var contact = await _organizationContext.Contacts.Include(i => i.EmailList).FirstOrDefaultAsync(x => x.Id == contactId);
