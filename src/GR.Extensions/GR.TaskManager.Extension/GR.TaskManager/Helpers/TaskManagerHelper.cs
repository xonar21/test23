...
namespace GR.TaskManager.Helpers
{
    public class TaskManagerHelper
    {
        ...
        internal static Task TaskMapper(UpdateTaskViewModel taskViewModel, Task dbTaskResult)
        {
            ...
            dbTaskResult.OrganizationId = taskViewModel.OrganizationId;
            dbTaskResult.TaskTypeId = taskViewModel.TaskTypeId;
            dbTaskResult.AgreementId = taskViewModel.AgreementId;
        ...
        internal static GetTaskViewModel GetTaskMapper(Task dbTaskResult, Guid? currentUserId = null)
        {
            var dto = new GetTaskViewModel
            {
                ...
                AgreementId = dbTaskResult.AgreementId,
                OrganizationId = dbTaskResult.OrganizationId,
            ...
            var organizationService = IoC.Resolve<ICrmOrganizationService>();
            var organizationRequest = organizationService.FindOrganizationByIdAsync(dto.OrganizationId).Result;
            if (organizationRequest.IsSuccess)
                dto.Organization = organizationRequest.Result;
