Vue.component('Header', {
	...
	<header>
        <nav class="navbar navbar-light bg-white border-bottom py-0 px-40 main-nav">
            ...
            <UserBlock :user="user" />
			<span class="unread-notify" v-if="getUserNotificationsFromStore.find(notification => !notification.isDeleted)">&nbsp;</span>
        </nav>
    </header>`,
    data() {
        return {
	...
	methods: {
		...
		setUserInfo() {
			customAjaxRequest(apiEndpoints.Users.GetCurrentUserInfo).then(user => {
				...
				this.$store.dispatch('getUserNotifications', user.id);
