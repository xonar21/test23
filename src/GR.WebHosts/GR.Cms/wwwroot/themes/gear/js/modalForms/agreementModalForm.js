Vue.component('AgreementModalForm', {
	template: `
	<div>
		<Modal :refreshInputs="refreshInputs" :modalProps="agreementModalProps" @newValue="emitModalValue" :inputsKey="modalKey"/>
		<Modal :refreshInputs="refreshInputs" :modalProps="sameOrgAgreementModal"/>
	</div>
	`,
	props: {
		editable: false
			...
			return {
				...
				formInputs: [
					...
					},
					{
						name: 'Select',
						props: {
							id: 'agreeementInput-contractTemplateId',
					...
					},
					{
						name: 'Select',
						props: {
							id: 'agreeementInput-organizationId',
							label: 'Organization',
							required: true,
					...
					},
					{
						name: 'Select',
						props: {
							id: 'agreeementInput-organizationAddressId',
					...
					},
					{
						name: 'Select',
						props: {
							id: 'agreeementInput-currencyCode',
							label: 'Currency',
					...
					},
					{
						name: 'Textarea',
						props: {
							id: 'agreeementInput-description',
							label: 'Description',
				...
				formSubmits: [
					{
						...
						props: {
							...
							onClick: () => {
								$(`#${this.agreementModalProps.id}`).modal("hide");
							}
						}
					},
					{
				...
				onSubmit: () => {
					this.checkForDuplicateAndAdd().then(id => {
						...
						$(`#${this.agreementModalProps.id}`).modal("hide");
						$(`#${this.sameOrgAgreementModal.id}`).modal("show");
						this.$emit('action');
					}).catch(() => {
						...
						props: {
							...
							onClick: () => {
								...
								this.updateEntity().then(() => {
									$(`#${this.sameOrgAgreementModal.id}`).modal("hide");
								});
							}
						}
					},
					{
						...
						props: {
							...
							onClick: () => {
								this.modalKey++;
								$(`#${this.sameOrgAgreementModal.id}`).modal("hide");
								$(`#${this.agreementModalProps.id}`).modal("show");
							}
						}
					},
					{
	...
	created() {
		...
		Promise.all(promises).then(result => {
			...
			this.currenciesListSelect = this.convertCurrenciesToSelectList(result[4]);
		});
	},
	methods: {
		...
		emitModalValue(val) {
			...
			this.modalValues[val.id.replace('agreeementInput-', '')] = newVal;
			if (val.id == 'agreeementInput-organizationId') {
