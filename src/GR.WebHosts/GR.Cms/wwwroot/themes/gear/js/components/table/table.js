...
Vue.component('Table', {
	...
	data() {
		return {
			...
			currentPage: parseInt(window.location.search.substr(4, window.location.search.length)),
			dataLoaded: false,
			loaderHeight: '150px',
			tableHeight: '150px',
			loadMoreCount: null,
	...
	methods: {
		onPaginationData(paginationData) {
			if (paginationData) {
				const last_page = Math.ceil(paginationData.rowCount / this.showPerPageInt);
				const newPaginationData = {
					total: paginationData.rowCount,
					per_page: this.showPerPageInt,
		...
		onChangePage(page) {
			if (page != this.currentPage && this.changeUrl) {
				window.history.pushState('', '', this.controller + "?nr=" + page);
			}
		...
		t(key) {
			return window.translate(key.toLowerCase().replace(/ /g, "_"));
		}
	},
	watch: {
		pageChange: function (newVal) {
			...
			console.log(window.location.pathname, '---', this.controller);
			
			if (window.location.pathname === '/' + this.controller
				|| window.location.pathname === this.controller) {
				this.currentPage = Number(urlParams.nr);
				this.$refs["vuetable"].reload();
			}
		},
		...
		showPerPage: function () {
			this.currentPage = parseInt(window.location.search.substr(4, window.location.search.length));
			this.$refs["vuetable"].refresh();
		}
	}
});
